...
const Sidebar =async  ({user}:{user:User}) => {
  ...
  return (
    <div className='hidden md:block w-[20%] h-fit border border-gray-100 bg-white rounded shadow-sm'>
        <div className='flex relative flex-col items-center'>
            <div className='w-full h-16 overflow-hidden'>
                {
                    user && (
                        ...
                        alt="banner"
                        width={200}
                        height={200} className='w-full h-full rounded-t'/>
                    )
                }
            </div>
            ...
            <div className='border-b border-b-gray-300'>
                <div className='p-2 mt-5 text-center'>
                    <h1 className="font-bold hover:underline cursor-pointer">{user?`${user?.firstName}${user?.lastName}`:"Not Logged In"}</h1>
